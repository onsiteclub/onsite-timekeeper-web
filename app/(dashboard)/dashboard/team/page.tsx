...
function getLocalName(userId: string): string | null {
  try {
    return localStorage.getItem(`worker_name_${userId}`);
  } catch {
    return null;
  }
}

function saveLocalName(userId: string, name: string) {
  try {
    localStorage.setItem(`worker_name_${userId}`, name);
  } catch {}
}
...
function EditableName({
  ...
  useEffect(() => {
    const local = getLocalName(userId);
  ...
  return isEditing ? (
    <input
      ...
      className="font-medium text-text-primary border-b-2 border-primary outline-none bg-transparent w-full"
    />
  ) : (
  ...
  return (
    <div className="max-w-lg mx-auto">
          ...
          {viewerGrants.map((grant) => (
            ...
            >
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-full bg-primary-light flex items-center justify-center flex-shrink-0">
                  <span className="text-primary font-semibold">
                    {(getLocalName(grant.owner_id) || grant.owner_profile?.full_name || 'U')[0].toUpperCase()}
                  </span>
                </div>
                <div className="min-w-0 flex-1">
                  <EditableName
                    userId={grant.owner_id}
                    originalName={grant.owner_profile?.full_name || 'Unknown User'}
              ...
              <button
                ...
                className="text-primary font-medium text-sm flex-shrink-0 ml-2"
              >
                View
              </button>
            </div>
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0">
                    <span className="text-text-secondary font-semibold">
                      {(getLocalName(grant.viewer_id) || grant.viewer_profile?.full_name || 'U')[0].toUpperCase()}
                    </span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <EditableName
                      userId={grant.viewer_id}
                      originalName={grant.viewer_profile?.full_name || 'Unknown User'}
                    />
                    <p className="text-xs text-text-muted">
      ...
      <WorkerHoursModal
        ...
        workerName={
          (viewingGrant ? getLocalName(viewingGrant.owner_id) : null) ||
          viewingGrant?.owner_profile?.full_name ||
          'Worker'
        }
      />
    </div>
